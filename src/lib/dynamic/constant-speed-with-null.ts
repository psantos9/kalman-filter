...
export default function constantSpeedWithNull(args: { staticCovariance?: number[][], obsDynaIndexes?: number[], nullGapModel?: string[], init }, observation) {
  ...
  if (!obsDynaIndexes) {
    ...
    obsDynaIndexes = Array.from({ length: l }).fill(0).map((_, i) => i)
  }
  ...
  init ||= {
    mean: Array.from({ length: obsDynaIndexes.length * 2 }).fill(0).map(() => [0]),
    covariance: diag(Array.from({ length: obsDynaIndexes.length * 2 }).fill(huge)),
    index: -1
  }

  nullGapModel ||= Array.from({ length: dimension }).fill(0).map(() => 'linear')
